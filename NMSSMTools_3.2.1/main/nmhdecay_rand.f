...
************************************************************************
      ...
      PARAMETER (NFL=12,NPROB=53,NPAR=25)
      ...
      DOUBLE PRECISION SQMQ3MIN,SQMQ3MAX
      DOUBLE PRECISION SQMU3MIN,SQMU3MAX
      DOUBLE PRECISION SQMD3MIN,SQMD3MAX
      DOUBLE PRECISION SQML3MIN,SQML3MAX
      DOUBLE PRECISION SQME3MIN,SQME3MAX
      
      DOUBLE PRECISION SQMU12MIN,SQMU12MAX
      DOUBLE PRECISION SQMD12MIN,SQMD12MAX
      DOUBLE PRECISION SQMQ12MIN,SQMQ12MAX
      DOUBLE PRECISION SQML12MIN,SQML12MAX
      DOUBLE PRECISION SQME12MIN,SQME12MAX
      
      DOUBLE PRECISION AU3MIN,AU3MAX
      DOUBLE PRECISION AD3MIN,AD3MAX
      DOUBLE PRECISION AE3MIN,AE3MAX
      DOUBLE PRECISION AE12MIN,AE12MAX
     ...
     . AKFLAG,ALFLAG
      ...
      COMMON/MINMAX/SQMQ3MIN,SQMQ3MAX
      COMMON/MINMAX/SQMU3MIN,SQMU3MAX
      COMMON/MINMAX/SQMD3MIN,SQMD3MAX
      COMMON/MINMAX/SQML3MIN,SQML3MAX
      COMMON/MINMAX/SQME3MIN,SQME3MAX

      COMMON/MINMAX/SQMQ12MIN,SQMQ12MAX
      COMMON/MINMAX/SQMU12MIN,SQMU12MAX
      COMMON/MINMAX/SQMD12MIN,SQMD12MAX
      COMMON/MINMAX/SQML12MIN,SQML12MAX
      COMMON/MINMAX/SQME12MIN,SQME12MAX
      
      COMMON/MINMAX/AU3MIN,AU3MAX
      COMMON/MINMAX/AD3MIN,AD3MAX
      COMMON/MINMAX/AE3MIN,AE3MAX
      COMMON/MINMAX/AE12MIN,AE12MAX
...
*   Reading of the input parameters
      ...
      WRITE(0,*)"MQ3",SQMQ3MIN, SQMQ3MAX
      WRITE(0,*)"MU3",SQMU3MIN, SQMU3MAX
      WRITE(0,*)"MD3",SQMD3MIN, SQMD3MAX
      WRITE(0,*)"ML3",SQML3MIN, SQML3MAX
      WRITE(0,*)"ME3",SQME3MIN, SQME3MAX
      WRITE(0,*)"MQ12",SQMQ12MIN, SQMQ12MAX
      WRITE(0,*)"MU12",SQMU12MIN, SQMU12MAX
      WRITE(0,*)"MD12",SQMD12MIN, SQMD12MAX
      WRITE(0,*)"ML12",SQML12MIN, SQML12MAX
      WRITE(0,*)"ME12",SQME12MIN, SQME12MAX
      WRITE(0,*)"AU3",AU3MIN, AU3MAX
      WRITE(0,*)"AD3",AD3MIN, AD3MAX
      WRITE(0,*)"AE3",AE3MIN, AE3MAX
      WRITE(0,*)"AE12",AE12MIN, AE12MAX
...
* Q 3
      IF(SQMQ3MIN.EQ.SQMQ3MAX)THEN
       PAR(7)=SQMQ3MIN
      ELSE
       PAR(7)=SQMQ3MIN+(SQMQ3MAX-SQMQ3MIN)*RAN2(IDUM)
      ENDIF
* U 3
      IF(SQMU3MIN.EQ.SQMU3MAX)THEN
       PAR(8)=SQMU3MIN
      ELSE
       PAR(8)=SQMU3MIN+(SQMU3MAX-SQMU3MIN)*RAN2(IDUM)
      ENDIF

* D 3
      IF(SQMD3MIN.EQ.SQMD3MAX)THEN
       PAR(9)=SQMD3MIN
      ELSE
...
* L 3
      IF(SQML3MIN.EQ.SQML3MAX)THEN
       PAR(10)=SQML3MIN
      ELSE
       PAR(10)=SQML3MIN+(SQML3MAX-SQML3MIN)*RAN2(IDUM)
      ENDIF

* E 3
      IF(SQME3MIN.EQ.SQME3MAX)THEN
       PAR(11)=SQME3MIN
      ELSE
       PAR(11)=SQME3MIN+(SQME3MAX-SQME3MIN)*RAN2(IDUM)
      ENDIF
...
* AE12
      IF(AE12MIN.EQ.AE12MAX)THEN
       PAR(25)=AE12MIN
      ELSE
       PAR(25)=AE12MIN+(AE12MAX-AE12MIN)*RAN2(IDUM)
      ENDIF

*********
* First and second gen masses
*********
* Q 1,2
      IF(SQMQ12MIN.EQ.SQMQ12MAX)THEN
       PAR(15)=SQMQ12MIN
      ELSE
       PAR(15)=SQMQ12MIN+(SQMQ12MAX-SQMQ12MIN)*RAN2(IDUM)
      ENDIF

* U 1,2
      IF(SQMU12MIN.EQ.SQMU12MAX)THEN
       PAR(16)=SQMU12MIN
      ELSE
       PAR(16)=SQMU12MIN+(SQMU12MAX-SQMU12MIN)*RAN2(IDUM)
      ENDIF

* D 1,2
      IF(SQMD12MIN.EQ.SQMD12MAX)THEN
       PAR(17)=SQMD12MIN
      ELSE
       PAR(17)=SQMD12MIN+(SQMD12MAX-SQMD12MIN)*RAN2(IDUM)
      ENDIF

* L 1,2
      IF(SQML12MIN.EQ.SQML12MAX)THEN
       PAR(18)=SQML12MIN
      ELSE
       PAR(18)=SQML12MIN+(SQML12MAX-SQML12MIN)*RAN2(IDUM)
      ENDIF

* E 1,2
      IF(SQME12MIN.EQ.SQME12MAX)THEN
       PAR(19)=SQME12MIN
      ELSE
       PAR(19)=SQME12MIN+(SQME12MAX-SQME12MIN)*RAN2(IDUM)
      ENDIF
***************
***************
...
*******************************************************************
      ...
      INTEGER I,NLINE,INL,ICH,IX,IVAL,Q2FIX,NMSFLAG
      INTEGER N0,NLOOP,NBER,NPAR,ERR,NTOT,ISEED
      INTEGER M1FLAG,M2FLAG,M3FLAG,MHDFLAG,MHUFLAG
      INTEGER AKFLAG,ALFLAG,OMGFLAG,MAFLAG,PFLAG
...
*      IF(IX.EQ.16) PAR(25)=VAL
       IF(IX.EQ.167) AE12MIN=VAL
       IF(IX.EQ.168) AE12MAX=VAL

* Setting ML1,2,3 min, max
*      IF(IX.EQ.32) PAR(18)=VAL**2
*      IF(IX.EQ.33) PAR(10)=VAL**2
       IF(IX.EQ.327) SQML12MIN=VAL**2
       IF(IX.EQ.328) SQML12MAX=VAL**2
       IF(IX.EQ.337) SQML3MIN=VAL**2
       IF(IX.EQ.338) SQML3MAX=VAL**2

* Setting ML1,2,3 min, max
*      IF(IX.EQ.35) PAR(19)=VAL**2
*      IF(IX.EQ.36) PAR(11)=VAL**2
       IF(IX.EQ.357) SQME12MIN=VAL**2
       IF(IX.EQ.358) SQME12MAX=VAL**2
       IF(IX.EQ.367) SQME3MIN=VAL**2
       IF(IX.EQ.368) SQME3MAX=VAL**2

* Setting MQ1,2,3 min, max
*      IF(IX.EQ.42) PAR(15)=VAL**2
*      IF(IX.EQ.43) PAR(7)=VAL**2
       IF(IX.EQ.427) SQMQ12MIN=VAL**2
       IF(IX.EQ.428) SQMQ12MAX=VAL**2
       IF(IX.EQ.437) SQMQ3MIN=VAL**2
       IF(IX.EQ.438) SQMQ3MAX=VAL**2

* Setting MU1,2,3 min, max
*      IF(IX.EQ.45) PAR(16)=VAL**2
*      IF(IX.EQ.46) PAR(8)=VAL**2
       IF(IX.EQ.457) SQMU12MIN=VAL**2
       IF(IX.EQ.458) SQMU12MAX=VAL**2
       IF(IX.EQ.467) SQMU3MIN=VAL**2
       IF(IX.EQ.468) SQMU3MAX=VAL**2

* Setting MD1,2,3 min, max
*      IF(IX.EQ.48) PAR(17)=VAL**2
*      IF(IX.EQ.49) PAR(9)=VAL**2
       IF(IX.EQ.487) SQMD12MIN=VAL**2
       IF(IX.EQ.488) SQMD12MAX=VAL**2
       IF(IX.EQ.497) SQMD3MIN=VAL**2
       IF(IX.EQ.498) SQMD3MAX=VAL**2
...
*      IF(SQMQ3MIN.EQ.1d99)THEN
*       WRITE(0,1)"MQ3_min MUST BE GIVEN IN BLOCK EXTPAR"
*       ERR=1
*      ENDIF
*      IF(SQMU3MIN.EQ.1d99 .AND. SQMU3MAX.NE.1d99)THEN
*       WRITE(0,1)"MU3_min MUST BE GIVEN IN BLOCK EXTPAR"
*       ERR=1
*      ENDIF
*      IF(SQMD3MIN.EQ.1d99)THEN
*       WRITE(0,1)"MD3_min MUST BE GIVEN IN BLOCK EXTPAR"
*       ERR=1
*      ENDIF
...
*   Set default values
      ...
      IF(SQMQ3MAX.EQ.1d99)SQMQ3MAX=SQMQ3MIN
      IF(SQMU3MAX.EQ.1d99)SQMU3MAX=SQMU3MIN
      IF(SQMD3MAX.EQ.1d99)SQMD3MAX=SQMD3MIN
      IF(SQML3MAX.EQ.1d99)SQML3MAX=SQML3MIN
      IF(SQME3MAX.EQ.1d99)SQME3MAX=SQME3MIN
      
      IF(SQMQ12MAX.EQ.1d99)SQMQ12MAX=SQMQ12MIN
      IF(SQMU12MAX.EQ.1d99)SQMU12MAX=SQMU12MIN
      IF(SQMD12MAX.EQ.1d99)SQMD12MAX=SQMD12MIN
      IF(SQML12MAX.EQ.1d99)SQML12MAX=SQML12MIN
      IF(SQME12MAX.EQ.1d99)SQME12MAX=SQME12MIN
...
*
     ...
     . MQGUT,MUGUT,MDGUT,ML3GUT,ME3GUT,MLGUT,MEGUT
      ...
      IF(IFAIL.EQ.0.OR.IFAIL.EQ.10.OR.IFAIL.EQ.11.OR.IFAIL.EQ.12)THEN
      ...
      IF(ITOT.EQ.NTOT.OR.IDIM.EQ.IMAX)THEN
       DO I=1,IDIM
        WRITE(6,10)(RES(I,J),J=1,27+53+1)
 10     FORMAT(81E16.8)
       ENDDO
