...
************************************************************************
      ...
      DOUBLE PRECISION ME3GUT,MLGUT,MEGUT,CMS_SUSY
...
!      WRITE(0,1234)MHUGUT,MHDGUT,MSGUT,MQ3GUT,MU3GUT,MD3GUT,MST1
...
*   Check for errors
 ...
 29   ERR=0
      ...
      IF(PAR(1).EQ.1d99)THEN
       ...
       ERR=1
      ENDIF
      IF(PAR(2).NE.1d99 .AND. PAR(4).EQ.1d99)THEN
       WRITE(0,1)"IF KAPPA IS GIVEN IN BLOCK EXTPAR"
       WRITE(0,1)"THEN MUEFF MUST BE AS WELL"
       ERR=1
      ENDIF
      IF(PAR(4).NE.1d99 .AND. PAR(2).EQ.1d99)THEN
       WRITE(0,1)"IF MUEFF IS GIVEN IN BLOCK EXTPAR"
       WRITE(0,1)"THEN KAPPA MUST BE AS WELL"
       ERR=1
      ENDIF
      IF(PAR(4).NE.1d99 .AND. MHDINP.NE.1d99)THEN
       WRITE(0,1)"BOTH MUEFF AND MHD^2 CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(PAR(4).NE.1d99 .AND. MHUINP.NE.1d99)THEN
...
*   Set MAFLAG, SCANFLAGS
      ...
      ELSE
       MAFLAG=-1
      ENDIF
      IF(M1INP.NE.1d99)M1FLAG=1
      IF(M2INP.NE.1d99)M2FLAG=1
      IF(M3INP.NE.1d99)M3FLAG=1
      IF(MHDINP.NE.1d99)MHDFLAG=1
      IF(MHUINP.NE.1d99)MHUFLAG=1
      IF(ALINP.NE.1d99)ALFLAG=1
      IF(AKINP.NE.1d99)AKFLAG=1
...
*********************************************************************      
     ...
     . BRBtaunumax,BRBtaunumin
      ...
      DOUBLE PRECISION SIG(5,11),SIGT(5,6),XENON100
...
c      PRINT*,ALGUT
c      PRINT*,AKGUT
c      PRINT*,DSQRT(MHUGUT)
c      PRINT*,DSQRT(MHDGUT)
c      PRINT*,DSQRT(MSGUT)
c      PRINT*,DSQRT(LGUT)
c      PRINT*,DSQRT(KGUT)
c      PRINT*,DSQRT(HTOPGUT)
c      PRINT*,PAR(1)
c      PRINT*,PAR(2)
c      PRINT*,PAR(3)
c      PRINT*,PAR(4)
...
c      CALL PRINTMASSES(6,0)
c      CALL HIGGSMASSES(6)
c      CALL DECAY2INFO('h1',6)
c      CALL DECAY2INFO('h2',6)
c      CALL DECAY2INFO('h3',6)
c      CALL DECAY2INFO('ha',6)
c      CALL DECAY2INFO('hb',6)
c      WRITE(0,*)""
c      WRITE(0,*)"Omega=",OMG,"  Xf=",Xf
c      WRITE(0,*)""
c      CALL O1CONTENTS(6)
c      WRITE(0,*)""
c      CALL PRINTCHANNELS(Xf,1.D-3,1.D-6,1,6)
c      WRITE(0,*)""
      ...
      WRITE(17,900) 2,"3.2.0      # Version number"
     ...
     . "# Excluded by top -> b H+, H+ -> W+ A1, A1 -> 2taus"
       IF(PROB(45).NE.0d0)
     . WRITE(17,900) 3,"# Excluded by A/H -> tautau MSSM (LHC)"
       IF(PROB(46).NE.0d0)
     . WRITE(17,900) 3,"# Excluded by H -> tautau SM (LHC)"
       IF(PROB(47).NE.0d0)
     . WRITE(17,900) 3,"# Excluded by WH -> bb SM (LHC)"
       IF(PROB(48).NE.0d0)
     . WRITE(17,900) 3,"# Excluded by H -> ZZ SM (LHC)"
       IF(PROB(49).NE.0d0)
     . WRITE(17,900) 3,"# Excluded by H -> WW SM (LHC)"
       IF(PROB(50).NE.0d0)
     . WRITE(17,900) 3,"# Excluded by H -> gammagamma SM (LHC)"
       IF(PROB(51).NE.0d0)
     . WRITE(17,900) 3,"# Excluded by t -> bH+ (LHC)"
       IF(PROB(52).NE.0d0)
     . WRITE(17,900) 3,"# Excluded by squark/gluino searches (LHC)"
       IF(PROB(53).NE.0d0)
     . WRITE(17,900) 3,"# Excluded by Xenon100"
     ...
     . WRITE(17,901) 62,PAR(2),"KAPPA AT THE SUSY SCALE"
      IF(ALINP.NE.A0)WRITE(17,901) 63,ALINP,"ALAMBDA AT THE GUT SCALE"
     ...
     .   "# Exp. 2 Sigma: 1.753E+01 < Delta Ms < 1.801E+01:"
      ...
      WRITE(17,899) "# Exp. 95% C.L.: BR(Bs->mu+mu-) < 4.5E-09:"
     ...
     .   XENON100(DABS(MNEU(1))),"):"
        ...
        WRITE(17,915)"# values used for sigma_piN,sigma_0",
     .  " (strange content of the proton)"
        WRITE(17,901) 30,sigmapiN,"sigma_piN"
        WRITE(17,901) 40,sigma0,"sigma_0"
      ENDIF
     ...
     .        "HTAU(MGUT,DR_bar)"
      ...
      WRITE(17,902) 1,FTGUT(1),"PG=MHU"
      WRITE(17,902) 2,FTGUT(2),"PG=MHD"
      WRITE(17,902) 3,FTGUT(3),"PG=MS"
      WRITE(17,902) 4,FTGUT(4),"PG=M0"
      WRITE(17,902) 5,FTGUT(5),"PG=M1"
      WRITE(17,902) 6,FTGUT(6),"PG=M2"
      WRITE(17,902) 7,FTGUT(7),"PG=M3"
      WRITE(17,902) 8,FTGUT(8),"PG=M12"
      WRITE(17,902) 9,FTGUT(9),"PG=ALAMBDA"
      WRITE(17,902) 10,FTGUT(10),"PG=AKAPPA"
      WRITE(17,902) 11,FTGUT(11),"PG=A0"
      WRITE(17,902) 12,FTGUT(12),"PG=XIF"
      WRITE(17,902) 13,FTGUT(13),"PG=XIS"
      WRITE(17,902) 14,FTGUT(14),"PG=MUP"
      WRITE(17,902) 15,FTGUT(15),"PG=MSP"
      WRITE(17,902) 16,FTGUT(16),"PG=M3H"
      WRITE(17,902) 17,FTGUT(17),"PG=LAMBDA"
      WRITE(17,902) 18,FTGUT(18),"PG=KAPPA"
      WRITE(17,902) 19,FTGUT(19),"PG=HTOP"
      WRITE(17,902) 20,FTGUT(20),"PG=G0"
      WRITE(17,902) 21,FTGUT(21),"PG=MGUT"
      WRITE(17,902) 22,FTGUT(22),"MAX"
      WRITE(17,914) 23,INT(FTGUT(23)),"IMAX"

      WRITE(17,899) "# "
      WRITE(17,899) "# REDUCED CROSS SECTIONS AT LHC"
      WRITE(17,902) 11,SIG(1,7),"VBF -> H1 -> tautau"
      WRITE(17,902) 12,SIG(1,8),"ggF -> H1 -> ZZ"
      WRITE(17,902) 13,SIG(1,9),"ggF -> H1 -> WW"
      WRITE(17,902) 14,SIG(1,10),"ggF -> H1 -> gammagamma"
      WRITE(17,902) 15,SIG(1,11),"VBF -> H1 -> gammagamma"
      WRITE(17,902) 21,SIG(2,7),"VBF -> H2 -> tautau"
      WRITE(17,902) 22,SIG(2,8),"ggF -> H2 -> ZZ"
      WRITE(17,902) 23,SIG(2,9),"ggF -> H2 -> WW"
      WRITE(17,902) 24,SIG(2,10),"ggF -> H2 -> gammagamma"
      WRITE(17,902) 25,SIG(2,11),"VBF -> H2 -> gammagamma"
      WRITE(17,902) 31,SIG(3,7),"VBF -> H3 -> tautau"
      WRITE(17,902) 32,SIG(3,8),"ggF -> H3 -> ZZ"
      WRITE(17,902) 33,SIG(3,9),"ggF -> H3 -> WW"
      WRITE(17,902) 34,SIG(3,10),"ggF -> H3 -> gammagamma"
      WRITE(17,902) 35,SIG(3,11),"VBF -> H3 -> gammagamma"
 ...
 1    WRITE(18,899) "# HIGGS + TOP BRANCHING RATIOS IN SLHA FORMAT"
      ...
      WRITE(18,900) 2,"3.2.0      # Version number"
 ...
 2    WRITE(19,899) "# RELIC DENSITY CALCULATED BY MICROMEGAS"
      ...
      WRITE(19,900) 2,"3.2.0      # Version number"
 ...
 914  FORMAT(1X,I9,3X,1P,I16,0P,3X,'#',1X,A)
 915  FORMAT(A,A)
