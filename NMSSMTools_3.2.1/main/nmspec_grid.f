...
************************************************************************
      ...
      PARAMETER (NFL=16,NPROB=53,NPAR=25)
      ...
      INTEGER NM1,NM2,NM3,NK,NMU,NTOT
      INTEGER IM0,IM12,ITB,IA0,IL,IAK,IAL,IMHD,IMHU
      INTEGER IM1,IM2,IM3,IK,IMU
      ...
      DOUBLE PRECISION A0MIN,A0MAX,M1MIN,M1MAX,M2MIN,M2MAX,M3MIN
      DOUBLE PRECISION M3MAX,MHDMIN,MHDMAX,MHUMIN,MHUMAX,LMIN,LMAX
      DOUBLE PRECISION ALMIN,ALMAX,AKMIN,AKMAX,KMIN,KMAX,MUMIN,MUMAX
      ...
      DOUBLE PRECISION MU3GUT,MD3GUT,CMS_SUSY
     ...
     . ALMIN,ALMAX,AKMIN,AKMAX,KMIN,KMAX,MUMIN,MUMAX
...
*   Check for errors
 ...
 29   ERR=0
      ...
      IF(MUMIN.EQ.1d99 .AND. MUMAX.NE.1d99)THEN
       ...
       ERR=1
      ENDIF
      IF(KMIN.NE.1d99 .AND. MUMIN.EQ.1d99)THEN
       WRITE(0,1)"IF KAPPA_MIN IS GIVEN IN BLOCK EXTPAR"
       WRITE(0,1)"THEN MUEFF_MIN MUST BE AS WELL"
       ERR=1
      ENDIF
      IF(MUMIN.NE.1d99 .AND. KMIN.EQ.1d99)THEN
       WRITE(0,1)"IF MUEFF_MIN IS GIVEN IN BLOCK EXTPAR"
       WRITE(0,1)"THEN KAPPA_MIN MUST BE AS WELL"
       ERR=1
      ENDIF
      IF(MUMIN.NE.1d99 .AND. MHDMIN.NE.1d99)THEN
       WRITE(0,1)"BOTH MUEFF_MIN AND MHD^2_MIN
     . CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(MUMIN.NE.1d99 .AND. MHUMIN.NE.1d99)THEN
       WRITE(0,1)"BOTH MUEFF_MIN AND MHU^2_MIN
     . CANNOT BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
...
*   Number of points
      ...
      IF(MAFLAG.NE.-5 .AND. NK.NE.1)THEN
       WRITE(0,1)"NK CANNOT BE GIVEN IN BLOCK STEPS"
       ERR=1
      ENDIF

      IF(MAFLAG.NE.-5 .AND. NMU.NE.1)THEN
     ...
     . .OR. NMHU.LE.0 .OR. NL.LE.0 .OR. NAL.LE.0 .OR. NAK.LE.0
     . .OR. NK.LE.0 .OR. NMU.LE.0)THEN
